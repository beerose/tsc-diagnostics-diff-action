...
async function capture(command: string, args: string[]): Promise<ExecResult> {
  ...
  try {
    ...
    const msg = `Command '${command}' failed with args '${args.join(' ')}': ${
      res.stderr
    }: ${err}`
...
export async function push(
  ...
  core.debug(
    `Executing 'git push' to branch '${branch}' with token and options '${options.join(
      ' '
    )}'`
  )
...
export async function pull(
  ...
  core.debug(
    `Executing 'git pull' to branch '${branch}' with token and options '${options.join(
      ' '
    )}'`
  )
...
export async function fetch(
  ...
  core.debug(
    `Executing 'git fetch' for branch '${branch}' with token and options '${options.join(
      ' '
    )}'`
  )
...
export async function clone(
  ...
  core.debug(
    `Executing 'git clone' to directory '${baseDirectory}' with token and options '${options.join(
      ' '
    )}'`
  )
